...
def main(args) :
    ...
    with open(args[0],'r') as edge_file :
        for line in edge_file :
            c1 = line.split()[0]
            c2 = line.split()[2]
            if f"{c1}:{c2}" in contig_set or f"{c1}:{c2}" in contig_set:
                read_set += ["@" + name for name in line.split()[4].split(";")]
    ...
    if len(read_set) != 0 :
        print("Some issue")
        print("Size of name list is " + str(len(read_set)))
