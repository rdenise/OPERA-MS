...
def kraken_binning(kraken_dict, kraken_out, output_file, abund_dict):
    cmd = f"cat {kraken_out} | cut -f1-4"
    kraken_result = subprocess.check_output(cmd, shell=True).decode('utf-8')
...
def main(args):
    ...
    contigs_bin_dict = kraken_binning(
        kraken_dict, args.kraken_out, args.output, abund_dict
    )
...
if __name__ == "__main__":
    ...
    mandatory.add_argument("-k", "--kraken_out", required=True, help="kraken out file")
    mandatory.add_argument(
        "-r", "--kraken_report", required=True, help="kraken out report file"
    )
    mandatory.add_argument(
        "-c",
        "--contig",
    ...
    mandatory.add_argument("-o", "--output", required=True, help="output file")
    mandatory.add_argument(
        "-a",
        "--abund",
        # required=True,
