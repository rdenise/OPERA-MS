...
from genfunc import create_dir, get_contig_file, run_exe
...
def get_long_read_file(read_file):
    """Return the long read file name"""
    res_file = read_file
    if not os.path.isfile(res_file):
        res_file = f"{res_file}.gz"
        if not os.path.isfile(res_file):
            sys.exit(f"Long read file not found : {read_file} or {res_file}")
    return res_file


def download_utils_db(db_type):
    """Download the database required by the utils command"""
    try:
        cmd = f"mkdir utils_db"
        run_exe(cmd, True)
    except:
        pass
    ...
    if db_type == "read-concordance":
        if not os.path.exists(("{}/../utils_db/kraken_db/hash.k2d".format(util_dir))):
            cmd = "wget -nc -P {}/../utils_db/ ftp://ftp.ccb.jhu.edu/pub/data/kraken2_dbs/minikraken_8GB_202003.tgz".format(
                util_dir
            )
            run_exe(cmd, True)
            cmd = "tar -xvzf {}/../utils_db/minikraken_8GB_202003.tgz -C {}/../utils_db".format(
                util_dir, util_dir
            )
            run_exe(cmd, True)
            cmd = (
                cmd
            ) = "mv {}/../utils_db/minikraken_8GB_20200312 {}/../utils_db/kraken_db".format(
                util_dir, util_dir
            )
            run_exe(cmd, True)
    ...
    if db_type == "novel-species":
        cmd = "wget --no-check-certificate -nc -O {}/../utils_db/GTDB.msh https://ndownloader.figshare.com/files/22471499".format(
            util_dir
        )
        run_exe(cmd, True)
        cmd = "wget --no-check-certificate -nc -O {}/../utils_db/MAG.msh https://ndownloader.figshare.com/files/22471505".format(
            util_dir
        )
        run_exe(cmd, True)
...
def read_taxonomy_file(
    genomes_dir, taxonomy, db_genome_dir, genome_list, genome_length
):
    ...
    for line in FILE:
        ...
        if species_name == "":
            ...
            novel_genome_name = (
                current_db_genomes_dir + "/" + species_name + "__" + genome
            )  # Need to fix this !!!
            # Check for gzip file
            ...
            OUT_LENGTH.write(
                "{}\t{}\n".format(
                    novel_genome_name,
                    "\t".join(
                        [str(x) for x in compute_genome_length(novel_genome_name)]
                    ),
                )
            )
            nb_genomes_in_dir += 1
...
def create_mash_sketch(genome_list, out_file, nb_thread):
    run_exe(
        "{}/mash sketch -o {} -p {} -l {}".format(
            util_dir, out_file, nb_thread, genome_list
        ),
        True,
    )  # other potential parameters -k -s
...
def run_circular_sequence_identification(assembly_dir):
    ...
    run_exe(
        util_dir
        + "/../bin/detect_circular_scaffold.pl "
        + contig_file
        + " "
        + scaffold_file
        + " "
...
def main(args):
    ...
    nb_thread = 0
    if (
        command == "read-concordance"
        or command == "circular-sequence"
        or command == "binning"
        or command == "bin-evaluation"
        or command == "mash"
    ):
        ...
        if command == "binning":
            ...
            if args.sample_name == None:
                sample_name = os.path.basename(
                    os.path.normpath(config_dict["OUTPUT_DIR"])
                )
            else:
            ...
            run_binner(
                ...
                config_dict["ILLUMINA_READ_1"],
                config_dict["ILLUMINA_READ_2"],
                nb_thread,
            )
            ...
            run_kraken2(
                ...
                config_dict["ILLUMINA_READ_1"],
                config_dict["ILLUMINA_READ_2"],
                get_long_read_file(config_dict["LONG_READ"]),
                nb_thread,
    ...
    # Command without config file
        ...
        if command == "novel-species":
            run_novel_species_analysis(
                args.known_species,
                args.taxonomy_database,
                args.thread,
                args.configs,
                args.binner,
                args.mags_qual,
...
def run_binner(binner, sample_name, assembly_dir, short_read1, short_read2, nb_thread):
    ...
    if binner == "metabat2":
        run_metabat2(
            contig_file, short_read1, short_read2, assembly_dir, sample_name, nb_thread
        )
...
if __name__ == "__main__":
    ...
    # opera-db
    opera_db_parser = subparsers.add_parser(
        "opera-ms-db", help="Generate a custom OPERA-MS genome database"
    )
    ...
    mandatory.add_argument(
        ...
        "--genomes-dir",
        required=True,
        help="Directory that contains genome files",
    )
    mandatory.add_argument(
        "-x", "--taxonomy", required=True, help="Species-level taxonomy of genomes"
    )
    mandatory.add_argument(
        "-d", "--db-name", required=True, help="Database name"
    )
    mandatory.add_argument(
        "-t", "--thread", help="Number of threads [default: 2]", default=2, type=int
    )

    # kraken
    kraken_parser = subparsers.add_parser(
        "read-concordance",
        parents=[config_parser.parser],
        help="Compute the abundance profile correlation between long and short-reads",
    )
    kraken_parser.add_argument(
        "-a",
        "--abundance-threshold",
        default=0.1,
        help="Lower percentage abundance threshold [default: 0.1]",
        type=float,
    )
    kraken_parser.add_argument(
        "-t", "--thread", help="Number of threads [default: 2]", default=2, type=int
    ...
    # binner
    binner_parser = subparsers.add_parser(
        "binning",
        parents=[config_parser.parser],
        help="Streamline binning of OPERA-MS assembled contigs using MetaBAT2 or MaxBin2",
    )
    binner_parser.add_argument(
        "-b",
        "--binner",
        required=False,
        default="metabat2",
        choices=["maxbin2", "metabat2"],
        help="binning method [default: metabat2]",
    )
    binner_parser.add_argument(
        "-s",
        "--sample-name",
        required=False,
        help="Sample name [default: OPERA-MS output folder]",
    )
    binner_parser.add_argument(
        "-t", "--thread", help="Number of threads [default: 2]", default=2, type=int
    )
    ...
    checkm_parser = subparsers.add_parser(
        "bin-evaluation",
        parents=[config_parser.parser],
        help="Streamline bin evaluation using CheckM",
    )
    checkm_parser.add_argument(
        ...
        required=False,
        default="metabat2",
        choices=["maxbin2", "metabat2", "opera_ms_clusters"],
        help="Bins for evaluation [default: MetaBat2]",
    )
    checkm_parser.add_argument(
        "-H",
        "--high-qual-mags",
        default="90,5",
        help="Completeness and contamination thresholds for high quality bins [default: 90,5]",
        type=str,
    )
    checkm_parser.add_argument(
        "-M",
        "--medium-qual-mags",
        default="50,10",
        help="Completeness and contamination thresholds for medium quality bins [default: 50,10]",
        type=str,
    )
    checkm_parser.add_argument(
        "-t", "--thread", help="Number of threads [default: 2]", default=2, type=int
    ...
    # novel species
    novel_species_parser = subparsers.add_parser(
        "novel-species",
        help="Identification of OPERA-MS MAGs most closely related species and identification of novel species",
    )
    novel_species_mandatory = novel_species_parser.add_argument_group("mandatory arguments")
    novel_species_mandatory.add_argument(
        "-o", "--out", required=True, help="Output directory"
    )
    #
    novel_species_parser.add_argument(
        "-k",
        "--known-species",
        required=False,
        default="{}/../utils_db/MAG.msh".format(util_dir),
        help=argparse.SUPPRESS,
    )  #'Mash sketch of known species reference genomes (default utils_db/small_newgut_segata.msh)')
    novel_species_parser.add_argument(
        "-x",
        "--taxonomy-database",
        required=False,
        default="{}/../utils_db/GTDB.msh".format(util_dir),
        help=argparse.SUPPRESS,
    )  #'Mash sketch of reference genomes with taxonomy info (default utils_db/genomes)')
    novel_species_parser.add_argument(
        "-b",
        "--binner",
        required=False,
        default="metabat2",
        choices=["maxbin2", "metabat2", "opera_ms_clusters"],
        help="Bins used for the analysis [default: MetaBat2]",
    )
    novel_species_parser.add_argument(
        "configs", metavar="C", nargs="+", help="Path to OPERA-MS configuration file(s)"
    )
    #
    novel_species_parser.add_argument(
        "-q",
        "--mags-qual",
        help="Quality of the MAGS used [default: high]",
        choices=["high", "medium"],
        default="high",
    )
    novel_species_parser.add_argument(
        "-c",
        "--cluster-threshold",
        help="Maximum distance at which 2 genomes are considered to be from the same species [default: 0.05]",
        default=0.05,
        type=float,
    )
    #
    ...
    # utils-db
    utils_db_parser = subparsers.add_parser(
        "utils-db", help=" Download the database required by the utils command"
    )
    mandatory = utils_db_parser.add_argument_group("mandatory arguments")
    utils_db_parser._action_groups[-1].add_argument(
        "-db",
        "--dbtype",
        choices=["read-concordance", "novel-species"],
        required=True,
        help="read-concordance or novel species analysis database",
    )

    # check if the utils sofware are functional in the current system
    check_install_parser = subparsers.add_parser(
        "check-dependency",
        help="Check which OPERA-MS-UTILS software are functional in the current system",
    )
